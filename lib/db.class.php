...
class db{
    ...
    public function __construct($once) {
        $cfg = self::instance($once);
        self::$dsn = $cfg['dsn'];
        self::$dbuser = $cfg['username'];
        self::$dbpass = $cfg['password'];
        self::$connect = true;
        self::$charset = 'UTF8';
        self::connect ();
     ...
     * *******************基本方法开始********************
     */
    static public function instance($once){
        if($once){
            global $config;
            return $config['db'][$once];
        }
    }
    /**
     * 作用:连結数据库
    ...
    public function connect() {
        try {
            self::$DB = new PDO ( self::$dsn, self::$dbuser, self::$dbpass, array (
    ...
    private function getPDOError($sql) {
        ...
        if (self::$DB->errorCode () != '00000') {
            ...
            echo (self::sqlError ( 'MySQL Query Error'.EOL, $info [2], $sql.EOL));
            exit ();
        }
    }
    ...
    private function errorfile($sql) {
        ...
        $errorfile = TEMP_PATH.'/sql/'.date('Ymd').'_error_log.txt';
        if (! file_exists ( $errorfile )) {
            $fp = file_put_contents ( $errorfile, date('Y-m-d')." MYSQL QUERY LOG\r\n".$log);
        } else {
            $fp = file_put_contents ( $errorfile, $log, FILE_APPEND);
        }
        flush();
    }
